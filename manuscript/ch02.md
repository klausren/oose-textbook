...
6. **Use AI** to draft a model comparison, then verify it against the
   chapter's risk-based checklist before trusting it.

## Before You Read  (~0.5 page)

Eight terms you will meet on nearly every page of this chapter. Learn them
now so the prose does not stall on vocabulary.

- **process** / 过程 — the agreed set of tasks: who does what, in what order, with what result.
- **process model** / 过程模型 — a template describing how a process is performed (which activities, in what order).
- **framework activity** / 框架活动 — one of the five universal building blocks: communication, planning, modeling, construction, deployment.
- **umbrella activity** / 伞形活动 — work that runs across all phases (review, risk management, SQA), not a phase itself.
- **process frame** / 过程框架 — a single activity described by entry criteria, task set, milestones, exit criteria.
- **milestone** / 里程碑 — a measurable checkpoint that answers "are we done with this activity?"
- **risk-driven** / 风险驱动 — choosing what to build next by what could most hurt the project.
- **prototype** / 原型 — a quick, throwaway build used to learn requirements, not to ship.
...
![Process model, framework activity and process frame nested inside one another](images/ch02-fig2-4-process-vocabulary.png){width=15cm}

*Fig. 2-4. The vocabulary of process, from the whole model down to the frame that describes a single activity.*
...
![The waterfall model: five sequential phases](images/ch02-fig2-1-waterfall.png){width=14cm}

*Fig. 2-1. The waterfall model. Work flows downward through sequential
phases; dashed arrows on the left show the feedback loops that send
problems back to an earlier phase.*
...
![The V model with requirements and design on the left leg and the matching tests on the right leg](images/ch02-fig2-5-v-model.png){width=15cm}

*Fig. 2-5. The V model. Every artefact on the left has a matching test on the right, and both are designed at the same time.*
...
![The incremental model: three full-process passes](images/ch02-fig2-2-incremental.png){width=15cm}
...
![The prototyping loop and the two roads out of it](images/ch02-fig2-6-prototyping.png){width=15cm}

*Fig. 2-6. Prototyping clarifies vague requirements fast, then offers two roads out — only one of which is usually safe.*
...
![The spiral model: four quadrants per loop](images/ch02-fig2-3-spiral.png){width=15cm}
...
![A decision table comparing five process models by fit, cost and suitability for CareLink](images/ch02-fig2-7-model-selection.png){width=15cm}

*Fig. 2-7. Model selection as an engineering decision: every choice buys something and pays for it.*
...
user feedback arrives after the course ends.

![The same sixteen-week project shown under waterfall and under incremental delivery](images/ch02-fig2-8-one-project-two-models.png){width=15cm}

*Fig. 2-8. The same project under two models. Waterfall defers reality to week 15; incremental invites it every four weeks.*
 A pure prototype is too small:
...
> **This chapter's AI angle:** AI can draft a model catalogue and recommend a
> process for a stated context — but it will recommend ceremony it cannot see
> the cost of, and it will invent models that do not exist.

**1. What AI is good at here**
- Turning a prose project description into a side-by-side comparison table of candidate models.
- Drafting a first-cut process frame (entry / task set / milestone / exit) from a vague brief.
- Generating a Gantt or activity diagram from a list of milestones.

![AI-assisted process selection with a human step that costs the ceremony](images/ch02-fig2-9-ai-model-selection.png){width=15cm}

*Fig. 2-9. AI-assisted process selection. Costing the ceremony is the step no model can do for you.*


**2. Prompt pattern** (copy-paste)

> Draft a process-model comparison for a project with: {{requirements clarity 1–5}},
> {{volatility}}, {{team size}}, {{risk level}}. For each of waterfall, incremental,
> spiral, agile: one strength, one weakness, and "choose if". Output as a table.
> Do not invent models. Mark any assumption as [ASSUMPTION].

**3. Verify before you trust**
- [ ] Every model named actually exists and is described correctly — no "Agile-Waterfall hybrid" you cannot define.
- [ ] The recommendation rests on the stated *risk*, not on which model has the longest Wikipedia page.
- [ ] The Gantt diagram's dependencies are real, not auto-arranged by date alone.
- [ ] Exit criteria are named, not just activities — AI loves listing tasks and skipping "done".

**4. Typical AI failure in this chapter**

*AI output:* "For a 6-person course project with uncertain requirements, use the
**Spiral model** — it is the most thorough."

*What is wrong:* Spiral's four-quadrant risk review every loop is *ceremony*; for
six students it costs more than the risk it controls (see Pitfall 4). A spiral
loop needs a disciplined team and a real risk to justify it.

*The repair:* recommend incremental + a short prototyping loop, and say why spiral
is oversized. Match the model to the risk profile, never to the model's reputation.
...
5. **Trusting an AI's model pick.** An AI will name the model with the most
   training data, not the one your risk profile earns. Verify the pick
   against the four-item checklist above before you write it into a plan.
...
Three sentence patterns this chapter's writing relies on — use them in your
process justification and lab write-up.

**Pattern 1 — Contrast, not contradiction** (*X is not Y; it is Z*)
> *The waterfall model is not "wrong"; it is fragile to change.*
Use it to correct a misconception without throwing the idea away.

**Pattern 2 — Fitness condition** (*Y suits Z when …*)
> *Waterfall suits projects whose requirements are stable and fixed by contract.*
State the condition under which a claim holds — the heart of "justify a choice."

**Pattern 3 — What varies** (*What differs between A and B is …*)
> *What differs between models is the order, frequency, and intensity of the five activities.*

**Practice** (write one sentence for each, about your own project):
1. "Scrum is not ______; it is ______."
2. "An incremental model suits our project when ______."
3. "What differs between our hybrid and pure waterfall is ______."
...
**(a) Concept checks**
1. Name the five framework activities and give a CareLink example of each.
2. Name three umbrella activities and explain why they cannot be scheduled as phases.
3. What are the four slots of a process frame, and why is "exit criteria" the one teams forget?
4. Define *milestone* in your own words, and give one example from the CareLink plan.

**(b) Analysis problems** — For each scenario, choose the most fitting model and defend it in two sentences: (1) a government contract with fully specified requirements and fixed acceptance tests; (2) a startup building a diet app whose target users keep changing what they want; (3) a medical-device firmware update where a failure could harm patients; (4) a university team of five with twelve weeks and a client who says "build me something like the campus safety app, but better."

**(c) AI-augmented tasks**
1. Paste your project context into the Ch-2 prompt pattern and let AI draft a model comparison. Then *verify* it against the checklist: cross out any invented model, and rewrite one recommendation so it rests on risk, not reputation. Hand in the AI output with your strikethroughs and one sentence on what you changed.
2. Ask AI to draw a Gantt chart for your three increments. Check that every dependency is real and that each phase names an exit criterion. List up to three defects you found and how you would fix them.

**(d) Running Project Task 2 — due Week 3**
